...
},{"./bundle-url":"../../../../../usr/local/lib/node_modules/parcel/src/builtins/bundle-url.js"}],"charts/AreaChart/areaChart.scss":[function(require,module,exports) {
var reloadCSS = require('_css_loader');

module.hot.dispose(reloadCSS);
module.hot.accept(reloadCSS);
},{"_css_loader":"../../../../../usr/local/lib/node_modules/parcel/src/builtins/css-loader.js"}],"charts/AreaChart/areaChart.js":[function(require,module,exports) {
"use strict";
...
var textColor = '#737070';
var circleColor = '#919191';
var circleInnerColor = 'white';
var textFontSize = '14px';
var yStrokeColor = '#282828';
var yStrokeDash = "6, 3";
var xAxisOffset = 20;
var wrapperWidth = 56;
var wrapperHeight = 40;
var getMonthFormat = d3.timeFormat("%b");

function addXGridGradient(svg) {
  var colorScale = d3.scaleLinear().range([textColor, '#12110F', 'black']).domain([1, 2, 3]);
  var linearGradient = svg.append("defs").append("linearGradient").attr("id", "area-chart__x-axis-gradient");
  linearGradient.append("stop").attr("offset", "0%").attr("stop-color", colorScale(1));
  linearGradient.append("stop").attr("offset", "75%").attr("stop-color", colorScale(2));
  linearGradient.append("stop").attr("offset", "100%").attr("stop-color", colorScale(3));
}

function addChartGradient(svg, uniqKey) {
  var gradient = ['white', 'rgba(117,117,117,0.1)'];
  var colorScale = d3.scaleLinear().range(gradient).domain([1, 2]);
  var linearGradient = svg.append("defs").append("linearGradient").attr("id", "area-chart__area-gradient".concat(uniqKey)).attr("gradientTransform", "rotate(90)");
  linearGradient.append("stop").attr("offset", "0%").attr("stop-color", colorScale(1));
  linearGradient.append("stop").attr("offset", "100%").attr("stop-color", colorScale(2));
}

function addLineGradient(svg, uniqKey) {
  var gradient = ['white', 'rgb(114,97,97)'];
  var colorScale = d3.scaleLinear().range(gradient).domain([1, 2]);
  var linearGradient = svg.append("defs").append("linearGradient").attr("id", "area-chart__line-gradient".concat(uniqKey)).attr("gradientTransform", "rotate(90)");
  linearGradient.append("stop").attr("offset", "0%").attr("stop-color", colorScale(1));
  linearGradient.append("stop").attr("offset", "100%").attr("stop-color", colorScale(2));
}

function removeAxisParts(_ref) {
  var svg = _ref.svg;
  var yAxis = svg.select('.area-chart__y-axis');
  yAxis.selectAll('text').remove();
  yAxis.selectAll('path').remove();
  yAxis.selectAll('g:first-of-type').remove();
  var xGridElement = svg.select(".area-chart__x-grid");
  xGridElement.select('path').remove();
  xGridElement.select('.tick').remove();
  xGridElement.selectAll('text').remove();
}

function createAxis(_ref2) {
  var svg = _ref2.svg;
  svg.append("g").attr("class", "area-chart__x-axis");
  svg.append("g").attr("class", "area-chart__x-grid").attr("stroke", "url(#area-chart__x-axis-gradient)");
  svg.append("g").attr("class", "area-chart__y-axis").style("stroke-dasharray", yStrokeDash);
}
...
function updateAxis(_ref3) {
  ...
  var xAxisElement = svg.select(".area-chart__x-axis").attr('transform', "translate(0, ".concat(height - xAxisOffset, ")")).call(xAxis);
  xAxisElement.select('path').attr('stroke', baseColor);
  xAxisElement.selectAll('line').attr('stroke', baseColor);
  xAxisElement.selectAll('text').attr('stroke', textColor).attr('fill', textColor).attr('font-size', textFontSize).attr("transform", "translate(-15, 10) rotate(-45)").text(function (x) {
    if (x.getMonth() === 0) return x.getFullYear();
    return getMonthFormat(x);
  });
  var xGridElement = svg.select(".area-chart__x-grid").attr("stroke", "url(#area-chart__x-axis-gradient)").call(xAxis.ticks(6).tickSize(height - xAxisOffset));
  xGridElement.selectAll('line').attr("stroke", yStrokeColor);
  xGridElement.selectAll('text').remove();
  var yAxisElement = svg.select(".area-chart__y-axis").style("stroke-dasharray", yStrokeDash).call(yAxis.tickValues([0, maxValue / 2 - maxValue / 20, maxValue - maxValue / 20]).tickSize(width));
...
function buildTooltip(_ref4) {
  var group = _ref4.group,
      x = _ref4.x,
      y = _ref4.y,
      tooltipValue = _ref4.tooltipValue,
      additionalClass = _ref4.additionalClass;
  var fo = group.append('foreignObject').style('pointer-events', 'none');
  var tooltip = fo.attr('width', wrapperWidth).attr('height', wrapperHeight).attr('x', x - wrapperWidth / 2).attr('y', y - wrapperHeight).append("xhtml:div");
  tooltip.classed('area-chart__tooltip', true).classed(additionalClass, true);
  tooltip.append('div').attr('class', 'area-chart__container').append('div').attr('class', 'area-chart__content').html(tooltipValue);
  tooltip.append('div').attr('class', 'area-chart__arrow');
}

function renderCircle(_ref5) {
  var cx = _ref5.cx,
      cy = _ref5.cy,
      svg = _ref5.svg;
  var circleGroup = svg.append('g').attr('class', "area-chart__circle-group 'area-chart__circle-group--active'");
  circleGroup.append('circle').attr('r', 5).attr('cx', cx).attr('cy', cy).attr('fill', circleColor);
  circleGroup.append('circle').attr('r', 2).attr('cx', cx).attr('cy', cy).attr('fill', circleInnerColor);
}

function findDataItem(data, datePoint) {
  var i = d3.bisector(function (d) {
    return d.label;
  }).left(data, datePoint); // returns the index to the current data item

  if (i === 0) return data[i];
  var d0 = data[i - 1].label;
  if (i >= data.length) return data[i - 1];
  var d1 = data[i].label; // work out which date value is closest to the mouse

  var realIndex = datePoint - d0 > d1 - datePoint ? i : i - 1;
  return data[realIndex];
}

function areaChart(_ref6) {
  var data = _ref6.data,
      _ref6$width = _ref6.width,
      width = _ref6$width === void 0 ? 510 : _ref6$width,
      _ref6$height = _ref6.height,
      height = _ref6$height === void 0 ? 255 : _ref6$height,
      selector = _ref6.selector,
      uniqKey = _ref6.uniqKey;
  var maxValue = Math.max.apply(Math, _toConsumableArray(data.map(function (x) {
    return x.value;
  })).concat([0]));
  var minValue = Math.min.apply(Math, _toConsumableArray(data.map(function (x) {
    return x.value;
  })).concat([0]));
  /*calculate metrics*/

  var xScale = d3.scaleTime().domain([data[0].label, data[data.length - 1].label]).range([0, width]);
  var yScale = d3.scaleLinear().range([height - 20, 0]).domain([minValue, maxValue + maxValue / 10]);
  var lineScale = d3.line().x(function (d) {
  ...
  var areaScale = d3.area().x(function (d) {
    return xScale(d.label);
  }).y0(height - 20).y1(function (d) {
    return yScale(d.value);
  });
  var chartContainer = d3.select(selector).append('svg').attr("class", "area-chart");
  addXGridGradient(chartContainer);
  addChartGradient(chartContainer, uniqKey);
  addLineGradient(chartContainer, uniqKey);
  var svg = chartContainer.data([data]).attr('width', width).attr('height', height).append('g');
  createAxis({
    svg: svg
  });
  updateAxis({
  ...
  svg.append('g').append('path').attr("class", "area-chart__area-gradient").datum(data).attr('fill', "url(#area-chart__area-gradient".concat(uniqKey, ")")).attr('d', areaScale);
  svg.append('g').append('path').attr("class", "area-chart__area-path").datum(data).attr('fill', 'none').attr('stroke', "url(#area-chart__line-gradient".concat(uniqKey, ")")).attr('stroke-width', '1px').attr('d', lineScale);
  var dataStore = {
    value: data,
    get: function get() {
    ...
    set: function set(value) {
      return dataStore.value = value;
    }
  };
  chartContainer.on('mouseover', function () {
    var gr = svg.append('g').attr('class', 'area-chart__tooltip-group');
    var mouseData = d3.mouse(this);
    var datePoint = xScale.invert(mouseData[0]);
    var item = findDataItem(dataStore.get(), datePoint);
    renderCircle({
    ...
    var datePoint = xScale.invert(mouseData[0]);
    var item = findDataItem(dataStore.get(), datePoint);
    var group = svg.select('.area-chart__tooltip-group');

    if (!group.select("[data-item=\"".concat(item.label, "\"]")).empty()) {
      return;
    }

    group.attr('data-item', item.label);
    group.selectAll('circle').transition().duration(50).ease(d3.easeLinear).attr('cx', xScale(item.label)).attr('cy', yScale(item.value));
    group.select('foreignObject').transition().duration(50).ease(d3.easeLinear).attr('x', xScale(item.label) - wrapperWidth / 2).attr('y', yScale(item.value) - wrapperHeight);
    group.select('.area-chart__content').html(item.tooltipValue);
  }).on('mouseout', function () {
    svg.select('.area-chart__tooltip-group').attr('class', '').transition().duration(400).style('opacity', 0).remove();
  });
  return function (newProps) {
    var newData = newProps.data || data;
    dataStore.set(newData);
    var maxValue = Math.max.apply(Math, _toConsumableArray(newData.map(function (x) {
      return x.value;
    })).concat([0]));
    var minValue = Math.min.apply(Math, _toConsumableArray(newData.map(function (x) {
      return x.value;
    })).concat([0]));
    chartContainer.data(newData).attr('width', newProps.width).attr('height', newProps.height);
    xScale.range([0, newProps.width]).domain([newData[0].label, newData[newData.length - 1].label]);
    yScale.range([newProps.height - 20, 0]).domain([minValue, maxValue + maxValue / 10]);
    areaScale.y0(newProps.height - 20);
    chartContainer.attr('width', newProps.width).attr('height', newProps.height);
    svg.select('.area-chart__area-gradient').datum(newData).transition().duration(100).attr('d', areaScale);
    svg.select('.area-chart__area-path').datum(newData).transition().duration(100).attr('d', lineScale);
    updateAxis({
      svg: svg,
...
},{"d3":"../node_modules/d3/index.js","./areaChart.scss":"charts/AreaChart/areaChart.scss"}],"charts/AreaChart/index.js":[function(require,module,exports) {
"use strict";
...
var _default = function _default(selector) {
  ...
  return {
    render: function render(data) {
      ...
      _update = (0, _areaChart.areaChart)({
        width: width,
        height: 0.5 * width,
        selector: selector,
        data: data,
...
exports.default = _default;
},{"./areaChart":"charts/AreaChart/areaChart.js"}],"app.scss":[function(require,module,exports) {
var reloadCSS = require('_css_loader');

module.hot.dispose(reloadCSS);
module.hot.accept(reloadCSS);
},{"_css_loader":"../../../../../usr/local/lib/node_modules/parcel/src/builtins/css-loader.js"}],"dummyData.js":[function(require,module,exports) {
"use strict";
...
exports.generateHorizontalBarData = generateHorizontalBarData;
...
var words = ["education", "veil", "boast", "vacation", "political", "rhetorical", "clumsy", "adhesive", "ladybug", "rock", "physical", "current", "thumb", "secretary", "ground", "gentle"];
...
function generateFakeData(count) {
  ...
  return sortedValues.reduce(function (acc, x) {
    var lastValue = acc.length > 0 ? acc[acc.length - 1].value : 0;
    acc.push(_objectSpread(_objectSpread({}, x), {}, {
...
function generateBarData(count) {
  ...
  var sum = res.reduce(function (acc, v) {
    return acc + v.value;
  }, 0);
...
function generateHorizontalBarData(count) {
  var res = [];
...
},{}],"charts/BarChart/barChart.scss":[function(require,module,exports) {
var reloadCSS = require('_css_loader');

module.hot.dispose(reloadCSS);
module.hot.accept(reloadCSS);
},{"_css_loader":"../../../../../usr/local/lib/node_modules/parcel/src/builtins/css-loader.js"}],"charts/BarChart/barChart.js":[function(require,module,exports) {
"use strict";
...
var FONT_SIZE = "14px";
var TEXT_COLOR = "#929292";
var GRID_COLOR = "#282828";
var yStrokeDash = "6, 3";
var BAR_COLORS = ["#311415", "#331313", "#361414", "#3a1414", "#3d1313", "#411415", "#441313", "#481313", "#4b1414", "#4d1313", "#521414", "#541414", "#581313", "#5b1414", "#5f1414", "#621414", "#651414", "#691414", "#6c1414", "#146F5F", "#157362", "#167665", "#157867", "#167C6A", "#16806D", "#17826F", "#168571", "#178A75", "#178C77", "#189079", "#18937C", "#19977F", "#199A82", "#199D84", "#1AA086", "#1BA389", "#1AA489", "#1BAA8F", "#1CAE91", "#1CB093", "#1DB496", "#1DB798", "#1EBA9B", "#1EBE9E", "#1FC1A1", "#20C5A4", "#20C8A6", "#20C9A7", "#20C9A7", "#20C9A7", "#20C9A7", "#20C9A7", "#20C9A7", "#20C9A7"];

function buildTooltip(_ref) {
  ...
  tooltip.append("div").attr("class", "bar-chart__container").append("div").attr("class", "bar-chart__content").html(percent.toFixed(2) + "%");
...
function createAxis(_ref2) {
  ...
  svg.append("g").attr("class", "bar-chart__y-axis").style("stroke-dasharray", yStrokeDash);
}

function renderAxis(_ref3) {
  var svg = _ref3.svg,
      ...
      data = _ref3.data;
  var xAxis = svg.select(".bar-chart__x-axis").attr("transform", "translate(0," + actualHeight + ")").call(d3.axisBottom(xScale));
  xAxis.select("path").remove();
  xAxis.selectAll("line").remove();
  xAxis.selectAll("text").attr("transform", "rotate(-90) translate( -25, -".concat(xScale.bandwidth() / 2 + 7, ")")).attr("text-anchor", "end").style("text-transform", "uppercase").style("cursor", "pointer").style("font-size", FONT_SIZE).style("fill", TEXT_COLOR).on("mouseover", function (position, i) {
    var label = data[i].label;
    svg.append("g").attr("data-type", "full-text").attr("transform", "translate(5," + yScale(0) + ")").append("text").attr("x", 0).attr("y", 15).attr("fill", TEXT_COLOR).style("font-size", FONT_SIZE).style("text-transform", "uppercase").attr("opacity", 0).text(label).transition().duration(200).attr("opacity", 1);
  }).on("mouseout", function () {
    svg.selectAll('[data-type="full-text"]').transition().duration(200).attr("opacity", 0).remove();
  });
  var yAxisElement = svg.select(".bar-chart___y-axis").style("stroke-dasharray", yStrokeDash).call(d3.axisRight(yScale).tickValues([maxValue * (1 / 3), maxValue * (2 / 3), maxValue]).tickSize(actualWidth));
  yAxisElement.selectAll("line").attr("stroke", GRID_COLOR);
  yAxisElement.selectAll("text").remove();
  yAxisElement.selectAll("path").remove();
}

function renderBars(_ref4) {
  var svg = _ref4.svg,
      xScale = _ref4.xScale,
      yScale = _ref4.yScale,
      data = _ref4.data,
      actualHeight = _ref4.actualHeight,
      maxValue = _ref4.maxValue,
      colorScale = _ref4.colorScale;
  var barPieSize = 5;
  var maxPieCount = (actualHeight - yScale(maxValue)) / barPieSize;
  colorScale.domain([0, maxPieCount]);
  var pieSize = barPieSize - 1;
  svg.selectAll('[data-type="bar-group"]').remove();
  data.forEach(function (x) {
    var barGroup = svg.append("g").attr("data-type", "bar-group");
    var actualPieCount = (actualHeight - yScale(x.value)) / barPieSize;
    var pieCount = Math.trunc(actualPieCount);
    var restPart = actualPieCount - Math.floor(actualPieCount);

    for (var i = 0; i < pieCount; i++) {
      barGroup.append("rect").attr("class", "bar-chart__bar-dash").attr("fill", colorScale(i)).attr("x", xScale(x.shortLabel)).attr("width", xScale.bandwidth()).attr("y", actualHeight - barPieSize * (i + 1)).attr("height", pieSize);
    }

    if (restPart !== 0) {
      barGroup.append("rect").attr("class", "bar-chart__bar-dash").attr("fill", colorScale(pieCount)).attr("x", xScale(x.shortLabel)).attr("width", xScale.bandwidth()).attr("y", actualHeight - barPieSize * (pieCount + 1) - (pieSize * restPart - barPieSize + 1)).attr("height", pieSize * restPart);
    }

    barGroup.append("rect").attr("class", "bar-chart__hidden-bar").attr("fill", "transparent").attr("x", xScale(x.shortLabel)).attr("width", xScale.bandwidth()).attr("y", yScale(x.value)).attr("height", actualHeight - yScale(x.value)).on("mouseover", function () {
      buildTooltip({
        group: barGroup,
...
function calculateMetrics(_ref5) {
  ...
  var yDomain = maxValue + maxValue / 20;
  return {
...
function barChart(_ref6) {
  var width = _ref6.width,
      height = _ref6.height,
      selector = _ref6.selector,
      data = _ref6.data,
      _ref6$margin = _ref6.margin,
      margin = _ref6$margin === void 0 ? {
    top: 0,
    left: 0,
    bottom: 90,
    right: 0
  ...
  var _calculateMetrics = calculateMetrics({
    width: width,
    height: height,
    margin: margin,
    data: data
  }),
      actualWidth = _calculateMetrics.actualWidth,
      actualHeight = _calculateMetrics.actualHeight,
      yDomain = _calculateMetrics.yDomain,
      maxValue = _calculateMetrics.maxValue;

  var xScale = d3.scaleBand().range([0, actualWidth]).padding(0.1).domain(data.map(function (x) {
    return x.shortLabel;
  }));
  var yScale = d3.scaleLinear().domain([0, yDomain]).range([actualHeight, 0]);
  var colorScale = d3.scaleLinear().range(['#311415', "#20C9A7"]);
  var svg = d3.select(selector).append("svg").data([data]).attr("width", actualWidth).attr("height", height).attr("class", "bar-chart");
  ...
  return function (_ref7) {
    var updatedData = _ref7.data,
        width = _ref7.width,
        height = _ref7.height;
    var newData = updatedData || data;

    var _calculateMetrics2 = calculateMetrics({
      width: width,
      height: height,
      margin: margin,
      data: newData
    }),
        actualWidth = _calculateMetrics2.actualWidth,
        actualHeight = _calculateMetrics2.actualHeight,
        yDomain = _calculateMetrics2.yDomain,
        maxValue = _calculateMetrics2.maxValue;

    xScale.range([0, actualWidth]).domain(newData.map(function (x) {
      return x.shortLabel;
    }));
    yScale.domain([0, yDomain]).range([actualHeight, 0]);
    svg.data([newData]).attr("width", actualWidth).attr("height", height);
    renderAxis({
      svg: container,
      actualHeight: actualHeight,
      yScale: yScale,
...
},{"d3":"../node_modules/d3/index.js","./barChart.scss":"charts/BarChart/barChart.scss"}],"charts/BarChart/index.js":[function(require,module,exports) {
"use strict";
...
exports.default = _default;
},{"./barChart":"charts/BarChart/barChart.js"}],"charts/ColumnBarChart/columnChart.scss":[function(require,module,exports) {
var reloadCSS = require('_css_loader');

module.hot.dispose(reloadCSS);
module.hot.accept(reloadCSS);
},{"_css_loader":"../../../../../usr/local/lib/node_modules/parcel/src/builtins/css-loader.js"}],"charts/ColumnBarChart/columnChart.js":[function(require,module,exports) {
"use strict";
...
var TEXT_COLOR = "#9B9B9B";
var BAR_COLORS = ["#0C6351", "#0B5545", "#19A887", "#20C9A7", "#58E2C2", "#27E9BE", "#5C9D92", "#5C9D92"].reverse();
var LEFT_OFFSET = 90;
var RIGHT_OFFSET = 70;

function renderBars(_ref) {
  var xScale = _ref.xScale,
      yScale = _ref.yScale,
      svg = _ref.svg,
      data = _ref.data;
  var barsGroup = svg.select('[data-type="bars-container"]');
  barsGroup.selectAll(".horizontal-bar-chart__bar").data(data).join("rect").attr("class", "horizontal-bar-chart__bar").attr("fill", function (d, index) {
    return BAR_COLORS[index];
  }).attr("width", function (d) {
    return xScale(d.value) < 10 ? 10 : xScale(d.value);
  }).attr("y", function (d) {
    return yScale(d.label) + yScale.bandwidth() / 2 - 5;
  }).attr("rx", "5px").attr("ry", "5px").attr("height", "10px").on("mouseover", function (d) {
    buildTooltip({
      group: barsGroup,
...
function buildTooltip(_ref2) {
  ...
  var fo = group.append("foreignObject");
  var wrapperWidth = 56;
  var wrapperHeight = 25;
  var tooltip = fo.attr("width", wrapperWidth).attr("height", wrapperHeight).attr("x", x + 15).attr("y", y - wrapperHeight / 2 + 5).style("overflow", "visible").append("xhtml:div");
  tooltip.classed("horizontal-bar-chart__tooltip", true).classed(additionalClass, true);
  tooltip.append("div").attr("class", "horizontal-bar-chart__container").append("div").attr("class", "horizontal-bar-chart__content").html(percent * 100);
...
function renderAxis(_ref3) {
  var svg = _ref3.svg,
      yScale = _ref3.yScale;
  var yAxis = svg.select('[data-type="y-axis"]').call(d3.axisLeft(yScale));
  yAxis.selectAll("path").remove();
  yAxis.selectAll("line").remove();
  yAxis.selectAll("text").attr("fill", TEXT_COLOR).attr("font-size", "12px");
}

function horizontalBarChart(_ref4) {
  var width = _ref4.width,
      height = _ref4.height,
      selector = _ref4.selector,
      data = _ref4.data;
  var actualWidth = width - LEFT_OFFSET - RIGHT_OFFSET;
  var maxValue = Math.max.apply(Math, _toConsumableArray(data.map(function (x) {
    return x.value;
  })));
  var xScale = d3.scaleLinear().range([0, actualWidth]).domain([0, maxValue]);
  var yScale = d3.scaleBand().range([height, 0]).padding(0.1).domain(data.map(function (x) {
    return x.label;
  }).reverse());
  var svg = d3.select(selector).append("svg").attr("width", width).attr("height", height).attr("class", "horizontal-bar-chart");
  var container = svg.append("g").attr("transform", "translate(".concat(LEFT_OFFSET, ")"));
  ...
  return function (updatedData) {
    var newData = updatedData.data || data;
    var maxValue = Math.max.apply(Math, _toConsumableArray(newData.map(function (x) {
      return x.value;
    })));
    var actualWidth = updatedData.width - LEFT_OFFSET - RIGHT_OFFSET;
    svg.attr("width", updatedData.width).attr("height", updatedData.height);
    xScale.range([0, actualWidth]).domain([0, maxValue]);
    yScale.range([updatedData.height, 0]).padding(0.1).domain(newData.map(function (x) {
      return x.label;
    }).reverse());
    renderAxis({
      svg: container,
...
},{"d3":"../node_modules/d3/index.js","./columnChart.scss":"charts/ColumnBarChart/columnChart.scss"}],"charts/ColumnBarChart/index.js":[function(require,module,exports) {
"use strict";
...
var _default = function _default(selector) {
  ...
  return {
    render: function render(data) {
      var container = document.querySelector(selector);
      var width = container.offsetWidth;
      _update = (0, _columnChart.horizontalBarChart)({
        width: width,
        height: 0.5 * width,
        selector: selector,
        data: data
...
exports.default = _default;
},{"./columnChart":"charts/ColumnBarChart/columnChart.js"}],"index.js":[function(require,module,exports) {
"use strict";

var _uuid = require("uuid");

var _AreaChart = _interopRequireDefault(require("./charts/AreaChart"));

require("./app.scss");

var _dummyData = require("./dummyData");

var _BarChart = _interopRequireDefault(require("./charts/BarChart"));

var _ColumnBarChart = _interopRequireDefault(require("./charts/ColumnBarChart"));
...
function initAreaChart() {
  var areaId = createChartContainer("Area chart");
  var area = (0, _AreaChart.default)("[data-id=\"".concat(areaId, "\"]"));
  var data = (0, _dummyData.generateFakeData)(100).map(function (_ref) {
    var time = _ref.time,
        value = _ref.value;
    return {
      label: time,
  ...
  setInterval(function () {
    var dataUpdate = (0, _dummyData.generateFakeData)(100).map(function (_ref2) {
      var time = _ref2.time,
          value = _ref2.value;
      return {
        label: time,
...
function initBarChart() {
  var barId = createChartContainer("Bar chart");
  var barChartComp = (0, _BarChart.default)("[data-id=\"".concat(barId, "\"]"));
  barChartComp.render((0, _dummyData.generateBarData)(15));
  window.addEventListener("resize", function () {
    barChartComp.update();
  });
}
...
function initHorizontalBarChart() {
  var barId = createChartContainer("Horizontal chart");
  var horizontalBarChartComp = (0, _ColumnBarChart.default)("[data-id=\"".concat(barId, "\"]"));
  horizontalBarChartComp.render((0, _dummyData.generateHorizontalBarData)(8));
  window.addEventListener("resize", function () {
...
},{"uuid":"../node_modules/uuid/dist/esm-browser/index.js","./charts/AreaChart":"charts/AreaChart/index.js","./app.scss":"app.scss","./dummyData":"dummyData.js","./charts/BarChart":"charts/BarChart/index.js","./charts/ColumnBarChart":"charts/ColumnBarChart/index.js"}],"../../../../../usr/local/lib/node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
